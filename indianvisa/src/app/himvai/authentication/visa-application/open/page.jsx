...
export default function Page() {
  ...
  useEffect(() => {
    const fetchDocumentsData = async () => {
      try {
        const { data } = await axiosInstance.get("/applicants/status/open");
        if (data?.success) {
    ...
    <div className="dark:bg-[#181818] dark:text-white bg-gray-100 text-black  min-h-screen">
      <div className="flex justify-between mb-4">
        ...
        <div className="flex gap-2">
          ...
          <button className="bg-yellow-500 text-white px-2 py-1 text-sm rounded border-2 dark:border-white border-yellow-600">
            Open
          </button>
          <button
            onClick={() =>
